//
// gosign - Go HTTP signing library for the Joyent Public Cloud and Joyent Manta
//
//
// Copyright (c) 2013 Joyent Inc.
//
// Written by Daniele Stroppa <daniele.stroppa@joyent.com>
//

package httpsignature

import (
	"net/http"
	"testing"
)

const (
	MantaSignature = ""
	testJpcKeyName = "test_key"
	key            = `-----BEGIN RSA PRIVATE KEY-----

cQYaFsx8Z1KwMVBTwDtiGXhd+82+dKnXxH4bZC+WAKs7L79HqhER
-----END RSA PRIVATE KEY-----`
)

func TestCreateAuthorizationHeader(t *testing.T) {
	req := &http.Request{}
	req.Header = make(http.Header)
	req.Header.Set("Date", "Mon, 14 Oct 2013 18:49:29 GMT")
	authentication, err := NewAuth(testJpcKeyName, key, "rsa-sha256")
	if err != nil {
		t.Error("Failed to create auth:", err)
		t.FailNow()
	}

	authHeader, err := CreateAuthorizationHeader(req, []string{"date"}, authentication)
	if err != nil {
		t.Error("Failed to sign the request")
	}
	if authHeader != "Signature keyId=\""+testJpcKeyName+"\",algorithm=\"rsa-sha256\",headers=\"date\",signature=\""+MantaSignature+"\"" {
		t.Error("Auth header does not conform to specification:", authHeader)
	}
}
